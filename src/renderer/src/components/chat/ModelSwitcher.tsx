...
function CustomModelIcon({ className }: { className?: string }): React.JSX.Element {
          ...
          >
            {selectedModel ? (
              <>
                ...
                <span className="truncate max-w-[200px] text-left" title={selectedModel.id}>
                  {selectedModel.name}
                </span>
              </>
            ) : (
        ...
        >
          <div className="flex min-h-[240px]">
            ...
            <div className="flex-1 p-2">
              ...
              ) : selectedProvider?.id === "openai_compatible" && !selectedProvider.hasApiKey ? (
                <div className="flex flex-col items-center justify-center h-[180px] px-4 text-center">
                  <Key className="size-6 text-muted-foreground mb-2" />
                  <p className="text-xs text-muted-foreground mb-3">
                    请添加至少一项自定义模型配置（接口地址与模型 ID）
                  </p>
                  <Button size="sm" onClick={() => setOpenAICompatDialogOpen(true)}>
                    添加配置
                  </Button>
                </div>
              ) : (
                ...
                <div className="flex flex-col h-[200px]">
                  <div className="overflow-y-auto flex-1 space-y-0.5">
                    {filteredModels.map((model) => {
                      const hideIdRow =
                        model.provider === "openai_compatible" || model.id.startsWith("oac:")
                      const showSecondLine = !hideIdRow && model.name !== model.id
                      return (
                        <button
                          key={model.id}
                    ...
                    {filteredModels.length === 0 && (
                      <p className="text-xs text-muted-foreground px-2 py-4">暂无可用模型</p>
                    )}
                  </div>
