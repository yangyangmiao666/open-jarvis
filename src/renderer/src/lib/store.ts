...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  colorMode:
    typeof window !== "undefined" && localStorage.getItem("openwork-theme") === "light"
      ? "light"
      : "dark",
    ...
    if (fromId) {
      try {
        ...
        if (prev) {
          ...
          if (merged.workspacePath === undefined && typeof prev.workspacePath === "string") {
  ...
  deleteThreads: async (threadIds: string[]) => {
    ...
    try {
      ...
      set((state) => {
        ...
        const wasCurrent = state.currentThreadId && idSet.has(state.currentThreadId)
